...
- [ ] **Dare accesso a una persona del team non si può fare** (trovato l'8 settembre verificando la sessione): `VenueMembership` la scrive solo il seed, e in tutto il prodotto non esiste una rotta che la crei o la cancelli. I cinque ruoli e le sette abilità funzionano, ma non c'è modo di assegnarli — un ristorante che compra Tavolo non può dare l'accesso al suo maître
      → **non serve la chiave email**: il manager può creare l'accesso e consegnare a voce una password provvisoria, oppure copiare un link d'invito, come già si fa col QR e col portale Wi-Fi. Quale delle due è una decisione di prodotto
