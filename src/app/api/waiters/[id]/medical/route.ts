...
import { FormazioneError, createMedicalCheck } from "@/server/staff-formazione";

export async function POST(req: Request, { params }: { params: { id: string } }) {
  const ctx = await requireVenueApi("manage_staff");
  if (!ctx.ok) return ctx.response;
  try {
    const created = await createMedicalCheck(ctx.venueId, params.id, await req.json(), auditActor(ctx, req));
