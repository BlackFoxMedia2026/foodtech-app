...
import {
  ArrivoAlPortale,
  AvanzateWifi,
} from "@/components/settings/wifi/azioni-portale";
    ...
    <div className="schermo animate-fade-in gap-3">
      ...
      <header className="fissa">
        ...
        <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
          Trasforma il Wi-Fi del locale in un nuovo punto di contatto con i tuoi
          clienti.
        </p>
      </header>
        ...
        <div className="space-y-5 pb-2">
          ...
          {stato.stato === "da_configurare" ? (
            ...
            <section className="riquadro comodo grid items-start gap-6 border-accent/30 bg-accent/[0.06] p-5 md:p-6 lg:grid-cols-[minmax(0,1fr)_240px]">
              <div className="space-y-5">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 space-y-1">
                    <h2 className="text-display text-xl">
                      Portale Wi-Fi non configurato
                    </h2>
                    <p className="max-w-2xl text-sm text-muted-foreground">
                      Bastano pochi passaggi per iniziare a raccogliere contatti
                      dal Wi-Fi del locale. Serve solo il nome della rete ospiti
                      e la sua password.
                    </p>
                  </div>
                </div>
                ...
                <ul className="space-y-2.5">
                  {[
                    ...
                    },
                    {
                      icona: Ticket,
                      testo:
                        "Se vuoi, chi si collega riceve anche uno sconto personale per tornare.",
                    },
                  ].map((riga) => (
                    <li
                      key={riga.testo}
                ...
                {canManage ? (
                  <Button asChild variant="accent">
                    <Link href="/settings/wifi/configura">
                      Configura il Wi-Fi
                    </Link>
                  </Button>
                ) : (
                  <p className="t-nota">
                    Il tuo ruolo non consente di configurare il portale.
                  </p>
                )}
              </div>
              ...
              <AnteprimaPortale
                dati={anteprima}
                className="hidden max-w-[240px] lg:block"
              />
            </section>
          ) : (
            <section className="riquadro comodo space-y-4 p-5 md:p-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0 space-y-2">
                  ...
                  {stats?.ultimoAccesso && (
                    <p className="t-nota">
                      Ultimo accesso: {daQuando(stats.ultimoAccesso)}.
                    </p>
                  )}
                  {stato.stato === "sospeso" && (
                    <p className="t-nota">
                      La pagina pubblica è spenta: si riaccende dalle
                      impostazioni avanzate, qui sotto.
                    </p>
                  )}
                </div>
          ...
          {stats && stats.contatti > 0 && (
            <>
              <div className="grid gap-3 sm:grid-cols-3">
                <Numero
                  valore={stats.ultimi30}
                  etichetta="Contatti"
                  base="negli ultimi 30 giorni"
                />
                <Numero
              ...
              <p className="flex flex-wrap items-center gap-x-3 gap-y-1 t-nota">
                ...
                <span>
                  Il numero che dice se questa cosa serve è «poi venuti a
                  mangiare»: un contatto raccolto non è un cliente.
                </span>
                <Link
                  href="/marketing/wifi"
                  className="inline-flex items-center gap-1 underline"
                >
                  <Users className="h-3.5 w-3.5" aria-hidden="true" /> Vedi chi
                  si è collegato
                </Link>
              </p>
            </>
          ...
          {stats && stats.contatti === 0 && stato.stato === "attivo" && (
            <p className="riquadro comodo flex items-start gap-2 text-sm text-muted-foreground">
              ...
              Nessuno si è ancora collegato. Il portale è acceso: quello che
              manca è far arrivare i clienti alla pagina — le due strade sono
              qui sotto.
            </p>
          )}
            ...
            <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
              <div className="space-y-5">
                ...
                */}
                {cifraturaAttiva() ? (
                  <p className="t-nota">
                    La password della rete è salvata cifrata: nel database non
                    c&apos;è il testo leggibile.
                  </p>
                ) : (
                  <p className="riquadro comodo border-accent/40 bg-accent/10 text-sm">
                    La password della rete è salvata in chiaro: questa
                    installazione non ha una chiave di cifratura configurata
                    (CHIAVE_CIFRATURA). Il portale funziona comunque.
                  </p>
                )}
              </div>
                ...
                <div>
                  <h2 className="t-titolo-scheda flex items-center gap-2">
                    <Wifi
                      ...
                      aria-hidden="true"
                    />{" "}
                    Come funziona il tuo portale
                  </h2>
                  <p className="t-nota">
                    Questa è la pagina che apre il cliente.
                  </p>
                </div>
