...
import { TelefonoPanel } from "@/components/insights/telefono-panel";

export const dynamic = "force-dynamic";

const PRESET_RANGE_KEYS = new Set([
  "last7",
  "last30",
  "last90",
  "currentMonth",
...
function computeRange(searchParams: {
  ...
  const key =
    searchParams.range && PRESET_RANGE_KEYS.has(searchParams.range)
      ? searchParams.range
      : "last7";
...
function trendFor(
  current: number,
  previous: number,
  opts?: { higherIsBetter?: boolean; kind?: "count" | "rate" },
) {
  ...
  return delta.available
    ? { value: delta.value, buono: delta.isGood }
    : undefined;
}
...
function DeltaBadge({
  ...
  if (!delta.available)
    return (
      <p className="text-xs text-muted-foreground">Confronto non disponibile</p>
    );
  if (delta.value === 0)
    return (
  ...
  return (
    <p
      className={cn(
        ...
        delta.isGood ? "text-sage-strong" : "text-destructive-soft",
      )}
    >
      {delta.value > 0 ? (
...
export default async function InsightsPage({
  ...
  */
  ...
  const [
    ...
    /* Il telefono si legge solo se il locale l'ha comprato, e la funzione
         che lo accende qui è `statistiche`: le altre due accendono il
         riconoscimento e le prenotazioni. Una licenza che non la comprende non
         deve far comparire un pannello vuoto in Analytics. */
    statoCentralino(ctx.venueId),
  ]);

  /* La lettura dei numeri del telefono si fa **dopo** e solo se serve: sono
     un'interrogazione in più, e su tutti i locali che non hanno il telefono —
     cioè quasi tutti — non si paga niente. */
  const numeriDelTelefono =
    centralino.attivo && centralino.funzioni.includes("statistiche")
      ? await numeriTelefono(ctx.venueId, from, to, ctx.venue.timezone)
      : null;
  ...
  const pctNewGuests = a.totalGuests
    ? Math.round((a.newGuests / a.totalGuests) * 100)
    : null;
  const pctRepeatGuests = a.totalGuests
    ? Math.round((a.repeatGuests / a.totalGuests) * 100)
    : null;
      ...
      <div className="fill-scroll space-y-4 pr-0.5">
        ...
        */}
              <StatCard
                ...
                value={
                  quotaAffidabile(a.bookings) ? `${a.occupancyRate}%` : "—"
                }
                hint={
                  quotaAffidabile(a.bookings)
                    ? "Prenotazioni completate"
                    : perchePercentualeAssente(a.bookings)
                }
                trend={
                  quotaAffidabile(a.bookings) && quotaAffidabile(prev.bookings)
                    ? trendFor(a.occupancyRate, prev.occupancyRate, {
                        kind: "rate",
                      })
                    : undefined
                }
              ...
              <StatCard
                ...
                hint={
                  quotaAffidabile(a.bookings)
                    ? "Sul totale prenotazioni"
                    : perchePercentualeAssente(a.bookings)
                }
                trend={
                  quotaAffidabile(a.bookings) && quotaAffidabile(prev.bookings)
                    ? trendFor(a.noShowRate, prev.noShowRate, {
              ...
              <StatCard
                ...
                trend={
                  quotaAffidabile(a.bookings) && quotaAffidabile(prev.bookings)
                    ? trendFor(a.cancelRate, prev.cancelRate, {
                        higherIsBetter: false,
                        kind: "rate",
                      })
                    : undefined
        ...
        */}
              <StatCard
                ...
                value={
                  a.ospitiConConti > 0
                    ? formatCurrency(a.avgSpendCents, ctx.venue.currency)
                    : "—"
                }
                hint={
                  a.ospitiConConti > 0
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Periodo selezionato rispetto a quello immediatamente
                  precedente
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid gap-3 md:grid-cols-3 lg:grid-cols-6">
                  ...
                  <ComparisonStat
                    label="Tasso completamento"
                    current={a.occupancyRate}
                    previous={prev.occupancyRate}
                    format={(v) => `${v}%`}
                    kind="rate"
                    nonDisponibile={
                      quotaAffidabile(a.bookings)
                  ...
                  <ComparisonStat
                    label="Cancellazioni"
                    current={a.cancelRate}
                    previous={prev.cancelRate}
                    format={(v) => `${v}%`}
                    higherIsBetter={false}
                    kind="rate"
                    nonDisponibile={
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Cosa dicono i dati di questo periodo
                </CardDescription>
              </CardHeader>
              <CardContent>
                {insights.length === 0 ? (
                  <p className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground">
                    Non ci sono ancora abbastanza dati per generare insight
                    affidabili.
                  </p>
                ) : (
        ...
        {vistaAttiva === "domanda" && (
          <>
            ...
            {/* Il telefono sta in «Domanda e ospiti» e non in una quinta vista:
              una telefonata è domanda che arriva da un altro canale, e le
              domande che si fanno a questa pagina sono quattro — non cinque.
              Sta sopra le fonti di prenotazione, che è la sua parente più
              vicina. */}
            {numeriDelTelefono && (
              <TelefonoPanel
                numeri={numeriDelTelefono}
                periodoGiorni={giorni}
              />
            )}
            <NpsPanel
              stats={nps}
              funnel={ponteRecensioni}
              giorni={giorniVoti}
            />
            <section className="grid gap-6 lg:grid-cols-2">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Distribuzione del flusso nel periodo selezionato
                  </CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Coperti per giorno della settimana e fascia oraria
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <WeekdayHeatmap data={a.heatmap} />
                </CardContent>
              </Card>
            </section>
            <Card>
            ...
            <section className="grid gap-4 md:grid-cols-2">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Profili creati nel periodo selezionato
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {pctNewGuests !== null
                      ? `${pctNewGuests}% degli ospiti nel periodo`
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Hanno visitato più di una volta
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-2">
                  <p className="text-display text-4xl">{a.repeatGuests}</p>
                  <p className="text-sm text-muted-foreground">
                    {pctRepeatGuests !== null
                      ? `${pctRepeatGuests}% degli ospiti nel periodo`
                      : "Nessun ospite nel periodo"}
                  </p>
                  <DeltaBadge
                    current={a.repeatGuests}
                    previous={prev.repeatGuests}
                  />
                  <Button asChild variant="outline" size="sm">
                    <Link href="/guests">
                      Vai a Ospiti <ArrowRight className="h-4 w-4" />
                    </Link>
                  </Button>
                </CardContent>
              </Card>
